...
layui.define(['layer','uploadEdit','dialog'], function(exports){
  "use strict";
              ...
              obj.preview(function(index, file, result){
                  uploadEdit({
                      ...
                      save:function (base64) {
                          if(_auto){
                              obj.upload(index, base64);
                          }else{
                              if(!that.files){
                                  that.files={};
                              }
                              that.files[index]=base64;
                          }
                      }
                  }).render(file)
              });
  ...
  Class.prototype.upload = function(files, type){
    ...
    ,check ,value = function(){
      ...
      layui.each(files || that.chooseFiles, function(i, item){
          var name=item.name;
          name=name? name :i+'.'+item.match(/\w*data:image\/\w*/).length>0?item.match(/\w*data:image\/\w*/)[0].split('/')[1]:'jpg';
        arr.push(name);
      });
      return arr;
    }()
    ...
    ,send = function(){
      ...
      value = value.length === 0
          ...
          :  value.length === 1?((elemFile.value.match(/[^\/\\]+\..+/g)||[]) || ''):value;
    ...
    if(options.number && that.fileLength > options.number){
      return $.msg('同时最多只能上传的数量为：'+ options.number);
    }
    ...
    if(options.size > 0 && !(device.ie && device.ie < 10)){
      ...
      if(limitSize) return $.msg('文件不能超过'+ limitSize);
    }
    send();
  };
