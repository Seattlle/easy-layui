layui.define(['jquery','util','dialog','md5'], function (exports) {
    window.$  =  layui.jquery;
    var util=layui.util,
        layer=layui.layer;
    ...
    var methods={
            ...
            var nodeName=_this.length>0? _this[0].nodeName.toLowerCase():_this.nodeName.toLowerCase()
                ,items={
                    ...
                    select:function (_params) {
                        var TIPS = '请选择', TITLE = 'layui-select-title'
                                ...
                                var showDown = function(){
                                    var rem=$(this)
                                        ...
                                        if(!list ||list.length===0){
                                            ...
                                            '</dl>'
                                                ,(function() {
                                                    var templet = select.attr('lay-templet')
                                                    if (templet) {
                                                        return '<div class="layui-anim layui-anim-upbit layui-select-extra">' + laytpl($(templet).html()).render({}) + '</div>'
                                                    }
                                                    return '';
                                                })()+'</div>'].join(''));
                         ...
                         return _select.each(function (index,item) {
                            ...
                            var value=params && params.value?params.value : othis.val();
                             othis.val(value);
                                ...
                                ,'<div class="'+ TITLE +'">' +
                                    function () {
                                        if(multiple){
                                            ...
                                            return '<input type="text" placeholder="'+ placeholder +'" value="'+ (value && selected.html() ? selected.html() : '') +'" '+ (isSearch ? '' : 'readonly') +' class="layui-select-label  layui-input'+ (isSearch ? '' : ' layui-unselect') + (disabled ? (' ' + DISABLED) : '') +'">';
                                        }
                                    }()
                    ...
                    ,cascader:function (options) {
                        var CLASS='layui-cascader',renderClass='layui-cascader-render',title='layui-select-title',childClass='cascader-menu-item',hasChildClass='cascader-menu-hasChild',ulClass='cascader-menu',ulWrap='layui-cascader-menu',
                            cascader=this.hasClass(CLASS)?this:this.find('.'+CLASS)
                            ,events=function(params){
                                var elem=$(this)
                                    ,renderList=function (data,leavel) {
                                        var $this=$(this),
                                    ...
                                    ,getArrayList= function (arr,leavel,params) {
                                        var li=[]
                                            ...
                                            ,value='value'
                                            ,childName='children'
                                            ,selected=isArrayFn(params.selected)?params.selected.splice(0,1)[0]:null;
                                ...
                                elem.off('click','.'+title).on('click','.'+title,function (e) {
                                    ...
                                    if(index===undefined){
                                        ...
                                        if(params.data){
                                            ...
                                            $.getJSON(params.url,function (res) {
                                                if(res.ok){
                                ...
                                body.off('click','.'+hasChildClass).on('click','.'+hasChildClass,function (e) {
                                    ...
                                    if(_params.url && _params.activeItemChange && _params.keyValue){
                                        ...
                                        actived[name]=_value;
                                        $.post(_params.url,actived,function (res) {
                                            if(res.ok){
                                ...
                                //点击叶子节点
                                body.off('click','.'+childClass).on('click','.'+childClass,function () {
                                    var othis=$(this),
                                        thisUl=othis.parent(),
                                        selectMenu=othis.parents('.'+ulWrap),
                                        _title=selectMenu.data('title'),
                                    ...
                                    if(!othis.hasClass(hasChildClass) || _params.changeOnSelect){
                                        ...
                                        var showValue=value.join(_params.split===undefined? '-':_params.split);
                        ...
                        cascader.each(function (index,item) {
                            ...
                            var elem=$(['<div class="'+renderClass+'">',
                                ...
                                '       <div class="'+title+'"><input type="text" placeholder="请选择" readonly value="'+selected+'" class="layui-input"><i class="layui-edge"></i></div>',
                                '   </div>',
                                '</div>'].join(''));
        ...
        getValue:function (params) {
            ...
            if(!methods.verify.call(this)) return ;
    ...
    $.fn.cascader  =  function (options) {
        var method  =  arguments[0];
        method=method==="setValue"?"cascaderSetValue":method;
        if (methods[method]) {
            method  =  methods[method];
            arguments  =  Array.prototype.slice.call(arguments, 1);
        } else if (typeof(method) === 'object' || !method) {
