layui.define(['jquery','laytpl','doT'],function(exports) {
    window.$ = layui.jquery;
    ...
    var dialog = {
      ...
      ready: function(callback){
        ...
        ,path = (isLayui ? 'modules/layer/' : 'theme/') + 'default/layer.css?v='+ dialog.v + ver;
    ...
    dialog.close = function(index){
        ...
        if((dialog.ie && dialog.ie < 10) || !layero.data('isOutAnim')){
            remove()
        } else {
            setTimeout(function(){
                remove();
    ...
    var Class = function(setings){
      ...
      if(that.config.url || that.config.template){
          var div = $('<div></div>')
              ,params={type:1};
          if(that.config.url){
              div.load(that.config.url, function(html) {
                  var text=$.doT(html,that.config.data||{});
                  $.extend(true, that.config, params, {
                      content: text
                  });
                  cssLinkWidth=$('#layuicss-layer').css('width');
                  document.body && cssLinkWidth==="1989px" ? that.creat() : setTimeout(function(){
                      that.creat();
                  }, 30);
              })
          }else{
              var text=$.doT(that.config.template,that.config.data||{});
              $.extend(true, that.config, params, {
                  content: text
              });
              cssLinkWidth=$('#layuicss-layer').css('width');
    ...
    Class.pt.vessel = function(conType, callback){
      ...
      callback([
        ...
        //主体
        '<div class="layui-layer-molv '+ doms[0] + (' layui-layer-'+ready.type[config.type]) + (((config.type == 0 || config.type == 2) && !config.shade) ? ' layui-layer-border' : '') + ' ' + (config.skin||'') +'" id="'+ doms[0] + times +'" type="'+ ready.type[config.type] +'" times="'+ times +'" showtime="'+ config.time +'" conType="'+ (conType ? 'object' : 'string') +'" style="z-index: '+ zIndex +'; width:'+ config.area[0] + ';height:' + config.area[1] + (config.fixed ? '' : ';position:absolute;') +'">'
          ...
          + (config.btn && config.btn.length>0 ? function(){
            var button = '';
    ...
    Class.pt.creat = function(){
      ...
      config.type == 2 && dialog.ie == 6 && that.layero.find('iframe').attr('src', content[0]);
    ...
    Class.pt.callback = function(){
      ...
      if(config.success){
        if(config.type == 2){
          layero.find('iframe').on('load', function(){//iframe加载完成后执行
    ...
    Class.pt.openLayer = function(){
      ...
      dialog.setTop = function(layero){
        var setZindex = function(){
          dialog.zIndex++;
          layero.css('z-index', dialog.zIndex + 1);
        };
        dialog.zIndex = parseInt(layero[0].style.zIndex);
    ...
    var cache = dialog.cache||{}, skin = function(type){
      ...
      return $.dialog($.extend({
        ...
        ,yes: function(index){
          ...
          if(value === ''){
            ...
            dialog.tips('&#x6700;&#x591A;&#x8F93;&#x5165;'+ (options.maxlength || 500) +'&#x4E2A;&#x5B57;&#x6570;', prompt, {tips: 1});
          } else {
    ...
    //     function(exports){
    //         dialog.path = layui.cache.dir;
    //         // ready.run(options);
    //     }()
    // ) : (
    //     (typeof define === 'function' && define.amd) ?function(){ //requirejs加载
    //         // ready.run(options);
    //         return dialog;
