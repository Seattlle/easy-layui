layui.define(['jquery','laydate','util','page','myform','tooltip','dialog'], function (exports) {
    ...
    var methods  =  {
        ...
        initBody: function (isFirst) {
            ...
            var c = $.data(_this, "datagrid"),
                ...
                table  =  [],
                cData=[].concat(c.data);
            var data  =  cData? cData.splice((c.request.page-1)*c.request.pageSize,c.request.pageSize):[];
        ...
        initStyle: function () {
            ...
            if (c.height||/^full(-\d+)*$/.test(c.height)) {
                ...
                table_h = c.page===undefined?_table_h:(table_h-41);
            ...
            if (isAllSet) {
                ...
                var leftWidth  =  totalWidth - setWidth - colsCount - 3;
                var _width  =  (leftWidth / unSetCount).toFixed(3)*1;
                ...
                $.ajax({
                    ...
                    success: function (_data) {
                        ...
                        var result  =  _data[util.webConfig.resName||c.response.resName],
                            data=result[util.webConfig.dataName || c.response.dataName];

                        c.response.total = result[util.webConfig.totalName||c.response.totalName];
                        if(c.hasSort){
                            $.each(data,function (i,k) {
                                k[indexName] = i;
                            })
        ...
        handleEditorChange:function(){
            var _this = this,
                ...
                if(_value && _value!=_old_value){
                    ...
                    c.editor&&c.editor(_text,c['data'][tid],field);
        ...
        scrollPatch:function(){
            ...
            if(c.autoColNums && outWidth < 5&&!c.scrollPatchWStatus){
                var th  =  header.eq(0).find('thead tr:last-child th:last-child'),

                    cutWidth = outWidth>0&&getScrollWidth>0?(getScrollWidth-outWidth):(getScrollWidth+outWidth),
