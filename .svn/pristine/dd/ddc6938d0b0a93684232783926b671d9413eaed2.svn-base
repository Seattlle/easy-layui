    ...
    var methods={
        ...
        initDom:function(){
            ...
            $.each(c.data,function (i,v) {
                ...
                li.push('<li class="'+ELEM_ITEM+function () {
                    if(c.vertical && $('body').find('.collapse').length>0){
                        if(v[childName]){
                            return ' hasChildMenu"';
                        }else{
                    ...
                    var name='<cite>'+v.name+'</cite>',iconName=c.iconName||'icon';
                    ...
                    if(c.showChild&&v[childName]){
                        ...
                        $.each(v[childName],function (ii,vv) {
                            child.push('<dd><a href="'+(vv.url?vv.url:'javascript:;')+'">'+(function () {
                                ...
                                if(c.showIcon && vv.icon){
                                    name='<i class="fa '+vv.icon+'"></i>'+name;
                                }
                                return name;
                            })()+'</a></dd>')
                        });
        ...
        event:function () {
            var that=this,
                ...
                selectedData={};
            //一级菜单
            $this.off('click').on('click','.'+ELEM_ITEM,function () {
                var _this=$(this),
                    c = $.data(that, "nav"),
                    index=_this.index(),
                    data=c.data[index],
            ...
            //二级菜单
            $this.on('click','.'+ELEM_CHILD+' dd',function (e) {
                e.stopPropagation();
                var _this=$(this),
                    c = $.data(that, "nav"),
                    childName=c.response.childName,
            ...
            $this.off('mouseenter').on('mouseenter','.'+ELEM_ITEM,function () {
                var _this=$(this),
            ...
            $this.off('mouseleave').on('mouseleave','.'+ELEM_ITEM,function () {
