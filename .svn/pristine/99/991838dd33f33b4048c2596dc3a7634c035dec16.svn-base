...
layui.define('jquery', function(exports){
  ...
  var call = {
    ...
    ,tabDelete: function(e, othis){
      ...
      ,parents = li.parents('.layui-tab').eq(0) //layui-tab
      ,item = parents.children('.layui-tab-content').children('.layui-tab-item')
      ,filter = parents.attr('lay-filter');

      if(li.hasClass(THIS)){
        if(li.next()[0]){
          call.tabClick.call(li.next()[0], null, index + 1);//触发下一个同级元素的点击事件
        } else if(li.prev()[0]){
    ...
    ,tabAuto: function(){
      ...
      $('.layui-tab').each(function(){
        ...
        if(title.prop('scrollWidth') > title.outerWidth()+1){
          ...
          title.append(span);//添加箭头按钮
          othis.attr('overflow', '');//超出隐藏
    ...
    ,clickThis: function(){
      ...
      if(othis.find('.'+NAV_CHILD)[0]) return; //如果有二级菜单则无法点击
    ...
    var items = {
      ...
      ,nav: function(){
        ...
        follow = function(bar, nav, index){
          var othis = $(this); //当前的li
          var child = othis.find('.'+NAV_CHILD);
          
          if(nav.hasClass(NAV_TREE)){ //判断是不是垂直或侧边导航
            bar.css({
        ...
        $(NAV_ELEM + elemFilter).each(function(index){
          ...
          itemElem.each(function(){
            var oitem = $(this);
            var child = oitem.find('.'+NAV_CHILD);         
            //二级菜单
